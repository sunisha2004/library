...
const BookDetail = () => {
  ...
  const token = localStorage.getItem("token");
  const [book, setBook] = useState(null);
  const [userRole, setUserRole] = useState(""); // Fetch user role dynamically
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (res.status === 200) {
          setUserRole(res.data.usr.accType.toLowerCase()); // Convert to lowercase for consistent comparison
        }
      } catch (error) {
  ...
  return (
    <div className="book-detail">
      <div className="book-container">
        ...
        <div className="buttons">
          {/* Show 'Book Now' button only if the user is not a Publisher */}
          {userRole !== "publisher" && (
            <button
              className="btn book-now"
              onClick={() => navigate(`/book/${bookId}/booking`)}
